...
export const merchantResolutionResultSchema = z.object({
  ...
  canonicalMerchantName: z.string().max(160).nullable(),
  canonicalProcessorName: z.string().max(160).nullable(),
  targetMerchantId: z.string().uuid().nullable(),
  targetProcessorId: z.string().uuid().nullable(),
  displayMerchantName: z.string().max(200).nullable(),
  ...
  categoryReason: z.string().max(240).nullable(),
})

export const categoryResolutionResultSchema = z.object({
  decision: z.enum(["link_existing_category", "create_custom_category"]),
  existingCategorySlug: z.string().max(120).nullable(),
  customCategoryName: z.string().max(120).nullable(),
  iconName: CATEGORY_ICON_NAME_SCHEMA.nullable(),
  colorToken: CATEGORY_COLOR_TOKEN_SCHEMA.nullable(),
  confidence: z.number().min(0).max(1),
